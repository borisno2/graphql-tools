...
import {BaseGraphQLError, ExecutionRef, GraphQLEndpointError, NestGraphQLEndpointParams} from './types'

interface NestedSource<TContext> {
  context: TContext,
  errors?: BaseGraphQLError[] | null,
  errorPromise?: Promise<BaseGraphQLError[]>
  resolveErrors?: (errors: GraphQLEndpointError[]) => void
}
...
const nestGraphQLEndpoint = <TContext>(params: NestGraphQLEndpointParams<TContext>) => {
  ...
  const transformedEndpointSchema = wrapSchema({
    ...
    createProxyingResolver: () => (parent: any, _args: any, _context: ExecutionRef, info: GraphQLResolveInfo) => parent[info.fieldName],
  ...
  return mergeSchemas({
    ...
    resolvers: {
      [parentType]: {
        [fieldName]: async (source: any, args: any, context: ExecutionRef, info: GraphQLResolveInfo) => {
