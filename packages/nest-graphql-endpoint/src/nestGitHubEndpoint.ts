...
const executor: Executor<{accessToken: string}> = async (
  document,
  variables,
  context
) => {
...
type NestGitHubParams = {prefix?: string} & Pick<NestGraphQLEndpointParams<{accessToken: string}>, 'parentSchema' | 'parentType' | 'fieldName' | 'resolveEndpointContext'>
